...
export default function IncomePage() {
  ...
  const handleDelete = async () => {
    try {
      ...
      showToast(error.response?.data?.message || "Failed to delete income", "error");
      setDeletingIncome(null);
    }
  };
    ...
    <>
              ...
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-5 sm:gap-6 items-stretch">
                <div className="bg-orange-50/40 rounded-2xl sm:rounded-[1.5rem] p-6 border border-orange-100 flex flex-col items-center justify-center gap-4 text-center h-full relative overflow-hidden">
                  ...
                  <div className="relative z-10 mt-2">
                    ...
                    <div className="mt-4 inline-flex py-1.5 px-4 rounded-xl text-sm font-bold bg-white text-orange-600 border border-orange-200 shadow-sm">{viewingIncome?.category?.name || "Unknown Category"}</div>
                  </div>
                </div>
                ...
                <div className="bg-slate-50 rounded-2xl sm:rounded-[1.5rem] p-6 border border-slate-100 flex flex-col justify-center h-full">
                  ...
                  <div className="space-y-4 sm:space-y-6">
                    ...
                    <div>
                      ...
                      <span className="inline-flex py-1 px-3 rounded-lg text-xs font-bold bg-slate-200 text-slate-700">{viewingIncome?.payment_method?.name || "Unknown Method"}</span>
                    </div>
                  </div>
                </div>
              </div>
                  ...
                  <div>
                    <div className="flex items-center justify-between pl-1 mb-1.5">
                      ...
                      {categoryOptions.length === 0 && (
                        <Link href="/categories" className="text-xs text-orange-600 hover:underline font-bold flex items-center gap-1">+ Add Category <ArrowRight size={12} /></Link>
                      )}
                    </div>
                  ...
                  <div>
                    <div className="flex items-center justify-between pl-1 mb-1.5">
                      ...
                      {methodOptions.length === 0 && (
                        <Link href="/payment-methods" className="text-xs text-orange-600 hover:underline font-bold flex items-center gap-1">+ Add Method <ArrowRight size={12} /></Link>
                      )}
                    </div>
      ...
      <div className="space-y-4 sm:space-y-6 animate-in fade-in zoom-in-95 duration-300">
        <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          {/* 🌟 只有打勾了 incomes.create 权限，才展示新增收入按钮 */}
          {can("incomes.create") && (
            <div className="flex items-center gap-2">
          ...
          <div className="p-4 sm:p-6 border-b border-orange-500/10 flex flex-col xl:flex-row xl:items-center justify-between gap-4 bg-white shrink-0">
            ...
            <div className="flex flex-col xl:flex-row items-center gap-3 w-full xl:w-auto xl:flex-1 xl:max-w-5xl xl:justify-end">
              ...
              <div className="hidden xl:flex items-center gap-2 shrink-0 pl-1">
                ...
                <button onClick={fetchIncomes} className="h-11 px-3 bg-orange-50 text-orange-500 hover:bg-orange-100 hover:text-orange-600 rounded-xl transition-colors flex items-center justify-center shrink-0 border border-transparent hover:border-orange-200" title="Refresh Table Data"><RefreshCw size={18} className={isLoading ? "animate-spin" : ""} /></button>
              </div>
            </div>
          </div>
                ...
                <div key={inc.id} className="bg-white p-4 rounded-2xl shadow-sm border border-orange-500/10 shadow-black/5 flex flex-col gap-3">
                  <div className="flex justify-between items-start">
                    <div className="flex items-center gap-3">
                      <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-orange-400 to-red-500 text-white flex items-center justify-center font-bold text-sm uppercase shrink-0">{getInitials(inc.title)}</div>
                      <div>
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-1 border-t border-orange-500/10 pt-3">
                    <span className="inline-flex py-1 px-2.5 rounded-lg text-[10px] sm:text-xs font-bold bg-orange-50 text-orange-600">{inc.category?.name || "Unknown"}</span>
                    <span className="inline-flex py-1 px-2.5 rounded-lg text-[10px] sm:text-xs font-bold bg-slate-100 text-slate-600">{inc.payment_method?.name || "Unknown"}</span>
                  </div>

                  {(can("incomes.view") || can("incomes.edit") || can("incomes.delete")) && (
          ...
          <div className="hidden lg:block overflow-x-auto custom-scrollbar">
            <table className="w-full text-left border-collapse min-w-[950px]">
              ...
              <tbody className="divide-y divide-orange-500/10">
                  ...
                  incomes.map((inc) => (
                    <tr key={inc.id} className="hover:bg-orange-50/40 transition-colors">
                      <td className="p-4 pl-6 font-bold text-sunset-dark">
                        <div className="flex items-center gap-3">
                          <div className="w-9 h-9 rounded-lg bg-gradient-to-br from-orange-400 to-red-500 text-white flex items-center justify-center font-bold text-sm uppercase shrink-0">{getInitials(inc.title)}</div>
                      ...
                      <td className="p-4 font-semibold text-sunset-dark/90 text-sm whitespace-nowrap">{inc.date}</td>
                      <td className="p-4"><span className="inline-flex py-1 px-2 rounded text-[10px] font-bold bg-orange-50 text-orange-700 whitespace-nowrap">{inc.time}</span></td>
                      <td className="p-4"><span className="inline-flex py-1 px-2.5 rounded-lg text-[10px] font-bold bg-slate-100 text-slate-600 whitespace-nowrap">{inc.payment_method?.name || "N/A"}</span></td>
                      <td className="p-4"><span className="inline-flex py-1 px-2.5 rounded-lg text-[10px] font-bold bg-orange-50 text-orange-600 whitespace-nowrap">{inc.category?.name || "N/A"}</span></td>
                ...
                {!isLoading && incomes.length === 0 && (
                  <tr><td colSpan={8} className="p-12 text-center text-sunset-dark/40 font-medium">No incomes found matching your search.</td></tr>
                )}
              </tbody>
            </table>
          </div>
