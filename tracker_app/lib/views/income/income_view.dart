...
class _IncomeViewState extends State<IncomeView> {
  ...
  final Color amountColor = const Color(0xFF059669);
  final int typeId = 2;
    ...
    try {
      ...
      setState(() {
        _categories = cats.where((c) => 
          ...
          (fieldInt(c, 'type_id', 1) == typeId || fieldText(c, 'type_name').toLowerCase().contains('income'))
        ).toList();

        _methods = methods.where((m) => 
          fieldInt(m, 'status', 1) == 1 && 
          (fieldInt(m, 'type_id', 1) == typeId || fieldText(m, 'type_name').toLowerCase().contains('income'))
        ).toList();
      });
    } catch (_) {}
  }
              ...
              child: Column(
                children: _items.asMap().entries.map((entry) {
                  ...
                  return Container(
                    ...
                    child: Row(
                      children: [
                        SizedBox(width: colTitle, child: Row(children: [ Container(width: 36, height: 36, decoration: BoxDecoration(color: SunsetColors.primary, borderRadius: BorderRadius.circular(10)), alignment: Alignment.center, child: Text(initials(fieldText(item, 'title'), 'IN'), style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 13))), const SizedBox(width: 12), Expanded(child: Text(fieldText(item, 'title'), style: const TextStyle(fontWeight: FontWeight.bold, color: SunsetColors.dark), overflow: TextOverflow.ellipsis)) ])),
      ...
      children: _items.map((item) {
        return Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              Container(width: 40, height: 40, decoration: BoxDecoration(color: SunsetColors.primary.withValues(alpha: 0.12), borderRadius: BorderRadius.circular(10)), alignment: Alignment.center, child: Text(initials(fieldText(item, 'title'), 'IN'), style: const TextStyle(color: SunsetColors.primary, fontWeight: FontWeight.bold))),
              const SizedBox(width: 12),
              Expanded(
                child: Column(
...
// 🌟 核心升级：带有模块化分区和清晰 Field Label 的 IncomeFormDialog
                    ...
                    Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          const Row(
                            children: [
                              Icon(Icons.account_balance_wallet_outlined, size: 18, color: SunsetColors.primary),
                              SizedBox(width: 8),
                          ...
                          _fieldLabel('TITLE'),
                          TextField(controller: _title, decoration: sunsetFieldDecoration('E.g. Salary, Freelance')),
                          const SizedBox(height: 14),
                            ...
                            Container(
                              ...
                              child: const Row(
                                children: [
                                  ...
                                  SizedBox(width: 8),
                                  Expanded(child: Text("No Income Category found. Please add a category first.", style: TextStyle(color: Color(0xFF92400E), fontSize: 12, fontWeight: FontWeight.bold))),
                                ],
                              ),
                            )
                          else
                            ...
                            Container(
                              ...
                              child: const Row(
                                children: [
                                  Icon(Icons.warning_amber_rounded, color: Color(0xFFD97706), size: 20),
                                  SizedBox(width: 8),
                                  Expanded(child: Text("No Income Payment Method found. Please add a method first.", style: TextStyle(color: Color(0xFF92400E), fontSize: 12, fontWeight: FontWeight.bold))),
                                ],
                              ),
                            )
                          else
                  ...
                  Expanded(
                    child: ElevatedButton(
                      onPressed: (hasNoCategories || hasNoMethods) ? null : () {
                        ...
                        Navigator.pop(context, _IncomeFormData(
