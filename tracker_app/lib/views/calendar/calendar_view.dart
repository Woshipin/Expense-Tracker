                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        Row(children: [Icon(Icons.schedule, size: 16, color: SunsetColors.dark.withValues(alpha: 0.5)), const SizedBox(width: 8), Text('CATEGORIZATION', style: TextStyle(color: SunsetColors.dark.withValues(alpha: 0.5), fontSize: 12, fontWeight: FontWeight.w900, letterSpacing: 1))]), 
                        const SizedBox(height: 16), 
                        
                        // 🌟 核心修复：给 Date 和 Time 包裹 Expanded 和 FittedBox 防止屏幕过窄时溢出
                        Row(children: [
                          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                            _label('Date'), 
                            InkWell(onTap: _pickDate, child: Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 13), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(10), border: Border.all(color: accent.withValues(alpha: 0.3))), child: Row(children: [Icon(Icons.calendar_today, size: 15, color: accent), const SizedBox(width: 4), Expanded(child: FittedBox(alignment: Alignment.centerLeft, fit: BoxFit.scaleDown, child: Text(_date, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700))))]))), 
                            if (_errorFor('date') != null) _errorText(_errorFor('date')!)
                          ])), 
                          const SizedBox(width: 12), 
                          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                            _label('Time'), 
                            InkWell(onTap: _pickTime, child: Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 13), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(10), border: Border.all(color: accent.withValues(alpha: 0.3))), child: Row(children: [Icon(Icons.access_time, size: 15, color: accent), const SizedBox(width: 4), Expanded(child: FittedBox(alignment: Alignment.centerLeft, fit: BoxFit.scaleDown, child: Text(_time, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700))))]))), 
                            if (_errorFor('time') != null) _errorText(_errorFor('time')!)
                          ]))
                        ]), 
                        
                        const SizedBox(height: 16), 
                        _label('Category'), DropdownButtonFormField<int>(initialValue: _categoryId, decoration: _fieldDecoration('Select Category'), items: widget.categoryOptions.isEmpty ? const [] : widget.categoryOptions.map((c) => DropdownMenuItem(value: c.id, child: Text(c.name))).toList(), hint: widget.categoryOptions.isEmpty ? const Text('Please add categories first', style: TextStyle(fontSize: 12)) : null, onChanged: (v) => setState(() => _categoryId = v)), if (_errorFor('category_id') != null) _errorText(_errorFor('category_id')!), const SizedBox(height: 16), _label('Payment Method'), DropdownButtonFormField<int>(initialValue: _paymentMethodId, decoration: _fieldDecoration('Select Method'), items: widget.methodOptions.isEmpty ? const [] : widget.methodOptions.map((m) => DropdownMenuItem(value: m.id, child: Text(m.name))).toList(), hint: widget.methodOptions.isEmpty ? const Text('Please add payment methods first', style: TextStyle(fontSize: 12)) : null, onChanged: (v) => setState(() => _paymentMethodId = v)), if (_errorFor('payment_method_id') != null) _errorText(_errorFor('payment_method_id')!)]),
                    ),
                  ],
                ),
              ),
